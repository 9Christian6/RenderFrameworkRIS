...
inline DirSample sample_cosine_hemisphere(const LocalCoords &coords, float u, float v)
{
    ...
    float theta = std::acos(std::sqrt(u));
    float phi = 2 * pi * v;
    float pdf = std::cos(theta) / pi;
    float x = std::cos(phi) * std::sin(theta);
    float y = std::sin(phi) * std::sin(theta);
    float z = std::cos(theta);
    float3 directionLocal{x, y, z};
    float3 directionWorld = coords.n * directionLocal.z + coords.t * directionLocal.x + coords.bt * directionLocal.y;
    return DirSample(normalize(directionWorld), pdf);
}
